...
public class TestProcesoMultiple {
	...
	// Usuario admin =
	// Repositorio.getInstance().usuarios().getUsuarioByName("admin");
	...
	// AuthAPI.getInstance().agregarFuncionalidad("actualizacionLocalesComerciales",
	// admin);
	//
	//
	// //---------------------------
	...
	// Usuario adminPrueba =
	// Repositorio.getInstance().usuarios().getUsuarioByName("adminPrueba");
	...
	// Usuario unUsuarioTerminal1 =
	// Repositorio.getInstance().usuarios().getUsuarioByName("terminal1");
	...
	// FuncAgregarAcciones funcion = ((FuncAgregarAcciones)
	// admin.getFuncionalidad("agregarAcciones"));
	...
	// FuncBajaPOIs funcion2 = ((FuncBajaPOIs)
	// admin.getFuncionalidad("bajaPOIs"));
	// Proceso proc2 = funcion2.prepDarDeBajaPOI(admin, tokenAdmin, 0, false,
	// (new File (".").getAbsolutePath ()) +
	// "/src/test/java/ar/edu/utn/dds/grupouno/procesos/bajaPois.json");
	// listProc.add(proc2);
	//
	// // agregamos un proceso actualizacionLocalesComerciales
	// FuncActualizacionLocalesComerciales funcion3 =
	// ((FuncActualizacionLocalesComerciales)
	// admin.getFuncionalidad("actualizacionLocalesComerciales"));
	...
	// FuncMultiple funcion1 = ((FuncMultiple)
	// admin.getFuncionalidad("procesoMultiple"));
	// funcion1.procesoMultiple(admin, tokenAdmin, 0, false, listProc);
	//
	// // Validaciones Proceso AgregarAcciones
	// // Se valida que el usuario adminPrueba tiene las funcionalidades
	// agregadas
	...
	//
	// // Se valida que el usuario unUsuarioTerminal1 tiene la funcionalidad
	// agregada
	...
	// POI local2Actualizado =
	// Repositorio.getInstance().pois().getPOIbyNombre("local2").get(0);
	// POI local3Actualizado =
	// Repositorio.getInstance().pois().getPOIbyNombre("local3").get(0);
	//
