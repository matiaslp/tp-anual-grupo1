...
public class TestActualizarLocalComercial {
	...
	// FuncActualizacionLocalesComerciales funcion =
	// (FuncActualizacionLocalesComerciales)
	// AuthAPI.getInstance().getAccion("actualizacionLocalesComerciales");
	// funcion.actualizarLocales(admin, tokenAdmin, 0, false, filePath);
	//
	// repositorio.usuarios().actualizarUsuario(); //ESTO DEBERIA SER UN UPDATE
	// GENERAL
	// //FALLA POR LA PARTE DE PROCESOS
	//
	// //Busco las modificaciones para corroborar que se corrio correctamente
	// POI local2Actualizado =
	// repositorio.pois().getPOIbyNombre("local2").get(0);
	// POI local3Actualizado =
	// repositorio.pois().getPOIbyNombre("local3").get(0);
	// POI local4Actualizado =
	// repositorio.pois().getPOIbyNombre("local4").get(0);
	//
